...
def balance_within_subj_vars(trial_parameters):
    ...
    trials = expand(trials, name='is_mask_overlap', values=[1,0],
                    ratio=trial_parameters['ratio_mask_overlap'], sample=False)
    trials = expand(trials, name='is_cue_masked', values=[1,0],
                    ratio=trial_parameters['ratio_cue_masked'], sample=False)
    trials['is_mask_overlap'][trials['is_cue_masked'] == 0] = -1
...
if __name__ == '__main__':
    ...
    version_file = os.path.join(exp_dir, 'temporal.yaml')
