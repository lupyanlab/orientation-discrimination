####################################################################################################
# data manipulation
library(dplyr)
library(reshape2)
library(car)

# plots
library(ggplot2)
library(scales)
library(grid)
####################################################################################################
df <- read.csv("./data/mwp5-2014-04-25.csv", stringsAsFactors = F)

(N <- length(unique(df$subj_id)))

df <- df[df$block_ix > -1, ]       # remove practice trials
df[df$is_correct == 0, "rt"] <- NA # remove RTs from incorrect and timeout trials
# df[df$response == "timeout", "is_correct"] <- NA # should timeout trials be considered incorrect?

####################################################################################################
subjs <- df %.% 
  group_by(subj_id) %.% 
  summarize(
    obs = length(rt),
    rt = mean(rt, na.rm = T),
    acc = mean(is_correct, na.rm = T)
  ) %.% 
  mutate(
    rank_rt = rank(rt, ties.method = "random"),
    rank_acc = rank(acc, ties.method = "random")
  )

ggplot(subjs, aes(x = rank_rt, y = rt, label = subj_id, color = subj_id)) +
  geom_point() +
  geom_text(hjust = 0, vjust = -0.5, angle = 45, size = 4) +
  coord_cartesian(xlim = c(0.5, 40), ylim = c(300, 800)) +
  scale_x_continuous("Subject (Ranked)", breaks = c(1, seq(5, 35, by = 5), 38)) +
  scale_y_continuous("Average RT (ms)") +
  theme(legend.position = "none")

# ggsave("./figures/temporal/subj-rt.png", width = 10, height = 5, units = "in")

ggplot(subjs, aes(x = rank_acc, y = acc, label = subj_id, color = subj_id)) +
  geom_point() +
  geom_text(hjust = 0, vjust = -0.5, angle = 45, size = 4) +
  coord_cartesian(xlim = c(0.5, 40), ylim = c(0.7, 1.05)) +
  scale_x_continuous("Subject (Ranked)", c(1, seq(5, 35, by = 5), 38)) +
  scale_y_continuous("Accuracy", label = percent) +
  theme(legend.position = "none")

# ggsave("./figures/temporal/subj-acc.png", width = 10, height = 5, units = "in")

####################################################################################################
pics <- df %.% 
  group_by(pic) %.%
  summarize(
    obs = length(unique(subj_id)),
    rt = mean(rt, na.rm = T),
    acc = mean(is_correct, na.rm = T)
  ) %.%
  mutate(
    rank_rt = rank(rt, ties.method = "random"),
    rank_acc = rank(acc, ties.method = "random")
  )

ggplot(pics, aes(x = rank_rt, y = rt, label = pic, color = pic, size = obs)) +
  geom_point() +
  geom_text(hjust = 0, vjust = -0.8, angle = 45, size = 4) +
  coord_cartesian(xlim = c(0.5, 40), ylim = c(300, 800)) +
  scale_x_continuous("Picture (Ranked)", breaks = c(1, seq(5, 35, by = 5), 38)) +
  scale_y_continuous("Average RT (ms)") +
  scale_size_continuous("# Subjects") +
  guides(color = "none") +
  theme(legend.position = c(0.9, 0.2),
        legend.background = element_blank())

# ggsave("./figures/temporal/pic-rt.png", width = 8, height = 5, units = "in")

ggplot(pics, aes(x = rank_acc, y = acc, label = pic, color = pic, size = obs)) +
  geom_point() +
  geom_text(hjust = 0, vjust = -0.5, angle = 45, size = 4) +
  coord_cartesian(xlim = c(0.5, 40), ylim = c(0.7, 1.05)) +
  scale_x_continuous("Picture (Ranked)", breaks = c(1, seq(5, 35, by = 5), 38)) +
  scale_y_continuous("Accuracy", label = percent) +
  scale_size_continuous("# Subjects") +
  guides(color = "none") +
  theme(legend.position = c(0.9, 0.2),
        legend.background = element_blank())

# ggsave("./figures/temporal/pic-acc.png", width = 8, height = 5, units = "in")
####################################################################################################
if (FALSE) { # shortcut to read data from the top
  df <- read.csv("./data/mwp5-2014-04-25.csv", stringsAsFactors = F)
  df <- df[df$block_ix > -1, ]       # remove practice trials
  df[df$is_correct == 0, "rt"] <- NA # remove RTs from incorrect and timeout trials
}

# make a data.frame of unique trial types
# used to code factors and to generate model predictions
trial_types <- expand.grid(cue_type = c("valid", "noise", "invalid"),
                           is_mask_overlap = c(-1, 0, 1))
trial_types$cue_m <- recode(trial_types$cue_type, "'valid' = -1/2; 'noise' = 0; 'invalid' = 1/2", as.factor.result = F)
trial_types$mask_c <- recode(trial_types$is_mask_overlap, "-1 = -1/2; else = 1/2", as.factor.result = F)
trial_types$mask_m <- recode(trial_types$is_mask_overlap, "-1 = -2/3; 0 = 1/3; 1 = 1/3", as.factor.result = F)
trial_types$mask_r <- recode(trial_types$is_mask_overlap, "-1 = 0; 0 = -1/2; 1 = 1/2", as.factor.result = F)

df <- merge(df, trial_types, by = c("cue_type", "is_mask_overlap"))

df <- df %.%
  filter(!(subj_id %in% c("MWP502", "MWP518", "MWP524", "MWP531"))) %.%
  select(subj_id, block_ix, trial_ix, 
         cue, cue_type, is_mask_overlap, 
         pic, up_pic, 
         response, rt, is_correct, 
         cue_m, mask_c, mask_m, mask_r) %.%
  arrange(subj_id, block_ix, trial_ix)

(N <- length(unique(df$subj_id)))

# write.csv(df, file = "./data/mwp5-final.csv", row.names = FALSE)
# df <- read.csv("./data/mwp5-final.csv")
####################################################################################################
# stats
library(lme4)
library(car)
library(AICcmodavg)
library(pbkrtest)

####################################################################################################
# Does the mask affect RTs?
if (FALSE) {
  set.seed(109)
  mask_rt <- lmer(rt ~ mask_c + (mask_c|subj_id), data = df)
  mask_rt_nomask <- update(mask_rt, . ~ . - mask_c)
  mask_rt_chi <- anova(mask_rt, mask_rt_nomask)
  mask_rt_anova <- NULL # KRmodcomp(mask_rt, mask_rt_nomask)
  
  save("mask_rt", "mask_rt_nomask", "mask_rt_chi", "mask_rt_anova",
       file = "./models/temporal/mask_rt.RData")
}

load("./models/temporal/mask_rt.RData")

summary(mask_rt)
mask_rt_chi
mask_rt_anova

# Does the mask affect accuracy?
if (FALSE) {
  set.seed(776)
  mask_acc <- glmer(is_correct ~ mask_c + (mask_c|subj_id), family = "binomial", data = df)
  mask_acc_nomask <- update(mask_acc, . ~ . - mask_c)
  mask_acc_chi <- anova(mask_acc, mask_acc_nomask)
  
  save("mask_acc", "mask_acc_nomask", "mask_acc_chi",
       file = "./models/temporal/mask_acc.RData")
}

load("./models/temporal/mask_acc.RData")

summary(mask_acc)
mask_acc_chi

####################################################################################################
# Does the effect of the cue type vary by masking condition?
if (FALSE) {
  set.seed(831)
  inter_rt <- lmer(rt ~ cue_m * mask_c + (cue_m * mask_c|subj_id), data = df)
  inter_rt_nointer <- update(inter_rt, . ~ . - cue_m:mask_c)
  inter_rt_chi <- anova(inter_rt, inter_rt_nointer)
  inter_rt_anova <- NULL # KRmodcomp(inter_rt, inter_rt_nointer)
  
  save("inter_rt", "inter_rt_nointer", "inter_rt_chi", "inter_rt_anova",
       file = "./models/temporal/inter_rt.RData")
}

load("./models/temporal/inter_rt.RData")

summary(inter_rt)
inter_rt_chi
inter_rt_anova

if (FALSE) {
  set.seed(112)
  inter_acc <- glmer(is_correct ~ cue_m * mask_c + (cue_m * mask_c|subj_id), 
                     family = "binomial", data = df)
  inter_acc_nointer <- update(inter_acc, . ~ . - cue_m:mask_c)
  inter_acc_chi <- anova(inter_acc, inter_acc_nointer)
  
  save("inter_acc", "inter_acc_nointer", "inter_acc_chi",
       file = "./models/temporal/inter_acc.RData")
}

load("./models/temporal/inter_acc.RData")

summary(inter_acc)
inter_acc_chi

####################################################################################################
# Does the effect of the mask vary by mask type?
if (FALSE) {
  set.seed(571)
  mtype_rt <- lmer(rt ~ cue_m * (mask_m + mask_r) + (1|subj_id), data = df)
  
  mtype_rt_nointer_m <- update(mtype_rt, . ~ . - cue_m:mask_m)
  mtype_rt_inter_m_chi <- anova(mtype_rt, mtype_rt_nointer_m)
  mtype_rt_inter_m_anova <- NULL # KRmodcomp(mtype_rt, mtype_rt_nointer_m)
  
  mtype_rt_nointer_r <- update(mtype_rt, . ~ . - cue_m:mask_r)
  mtype_rt_inter_r_chi <- anova(mtype_rt, mtype_rt_nointer_r)
  mtype_rt_inter_r_anova <- NULL # KRmodcomp(mtype_rt, mtype_rt_nointer_m)
  
  save("mtype_rt", "mtype_rt_nointer_m", "mtype_rt_inter_m_chi", "mtype_rt_inter_m_anova",
       "mtype_rt_nointer_r", "mtype_rt_inter_r_chi", "mtype_rt_inter_r_anova",
       file = "./models/temporal/mtype_rt.RData")
}

load("./models/temporal/mtype_rt.RData")

summary(mtype_rt)
mtype_rt_inter_m_chi
mtype_rt_inter_r_chi

if (FALSE) {
  set.seed(881)
  mtype_acc <- glmer(is_correct ~ cue_m * (mask_m + mask_r) + (1|subj_id), family = "binomial", data = df)
  
  mtype_acc_nointer_m <- update(mtype_acc, . ~ . - cue_m:mask_m)
  mtype_acc_inter_m_chi <- anova(mtype_acc, mtype_acc_nointer_m)
  
  mtype_acc_nointer_r <- update(mtype_acc, . ~ . - cue_m:mask_r)
  mtype_acc_inter_r_chi <- anova(mtype_acc, mtype_acc_nointer_r)
  
  save("mtype_acc", "mtype_acc_nointer_m", "mtype_acc_inter_m_chi", 
       "mtype_acc_nointer_r", "mtype_acc_inter_r_chi",
       file = "./models/temporal/mtype_acc.RData")
}

load("./models/temporal/mtype_acc.RData")

summary(mtype_acc)
mtype_acc_inter_m_chi
mtype_acc_inter_r_chi

####################################################################################################
if (FALSE) {
  load("./models/temporal/inter_rt.RData")
  
  trial_types <- expand.grid(cue_type = c("valid", "noise", "invalid"),
                             is_mask_overlap = c(-1, 0, 1))
  trial_types$cue_m <- recode(trial_types$cue_type, "'valid' = -1/2; 'noise' = 0; 'invalid' = 1/2", as.factor.result = F)
  trial_types$mask_c <- recode(trial_types$is_mask_overlap, "-1 = -1/2; else = 1/2", as.factor.result = F)
  trial_types$mask_m <- recode(trial_types$is_mask_overlap, "-1 = -2/3; 0 = 1/3; 1 = 1/3", as.factor.result = F)
  trial_types$mask_r <- recode(trial_types$is_mask_overlap, "-1 = 0; 0 = -1/2; 1 = 1/2", as.factor.result = F)
}

y_preds <- predictSE.mer(inter_rt, newdata = trial_types, se.fit = T, type = "response")
preds <- cbind(trial_types, y_preds)
preds$upr <- with(preds, fit + se.fit)
preds$lwr <- with(preds, fit - se.fit)

ggplot(preds, aes(y = fit, ymin = lwr, ymax = upr, x = mask_c, color = cue_type)) +
  geom_pointrange(position = position_dodge(width = -0.1), size = 1.0) +
  geom_line(position = position_dodge(width = -0.1), size = 1.0) +
  coord_cartesian(xlim = c(-1, 1), ylim = c(400, 600)) +
  scale_y_continuous("Response Time (ms)") +
  scale_x_continuous("", breaks = c(-0.5, 0.5), labels = c("No Mask", "Mask")) +
  scale_color_discrete("Cue Type", labels = c("Valid", "Noise", "Invalid")) +
  guides(color = guide_legend(reverse = T)) +
  theme(text = element_text(color = 'black', size = 18),
        line = element_line(color = 'black'),
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = 'gray', linetype = 3, size = 0.6),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = 'black'),
        axis.text = element_text(color = 'black'),
        axis.ticks.x = element_blank(),
        axis.ticks.y = element_line(color = 'black'),
        axis.ticks.length = unit(8, 'points'),
        legend.position = c(0.16, 0.88),
        legend.background = element_blank(),
        legend.key = element_blank())

ggsave("figures/temporal/reactiontime1.png", width = 6, height = 6, units = "in")

####################################################################################################

fixed_efs <- as.data.frame(summary(inter_rt)$coefficients)[1:2]
colnames(fixed_efs) <- c("beta", "se")
fixed_efs$param <- row.names(fixed_efs)
row.names(fixed_efs) <- NULL

rand_efs <- coef(inter_rt)$subj_id
rand_efs$subj_id <- row.names(rand_efs)
row.names(rand_efs) <- NULL

rand_efs$r1 <- rank(rand_efs[,1], ties.method = "random")
rand_efs$r2 <- rank(rand_efs[,2], ties.method = "random")
rand_efs$r3 <- rank(rand_efs[,3], ties.method = "random")
rand_efs$r4 <- rank(rand_efs[,4], ties.method = "random")

rand_efs <- melt(rand_efs, measure.vars = c("(Intercept)", "cue_m", "mask_c", "cue_m:mask_c"), 
                 id.vars = c("subj_id", "r1", "r2", "r3", "r4"), variable.name = "param", value.name = "beta")

ggplot(rand_efs, aes(y = beta)) +
  geom_point(aes(x = r1, color = subj_id)) +
  geom_pointrange(aes(x = 17.5, ymin = beta - se, ymax = beta + se), data = fixed_efs, size = 1.2) +
  coord_cartesian(xlim = c(0.5, 34.5)) +
  scale_x_continuous("Subject (Ranked)", breaks = c(1, seq(5, 30, by = 5), 34)) +
  scale_y_continuous("Parameter Estimate") +
  geom_hline(yintercept = 0, lty = 2) +
  facet_grid(param ~ ., scales = "free_y") +
  theme(text = element_text(color = 'black', size = 18),
        line = element_line(color = 'black'),
        panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_line(color = 'gray', linetype = 3, size = 0.6),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = 'black'),
        axis.text = element_text(color = 'black'),
        axis.ticks.x = element_blank(),
        axis.ticks.y = element_line(color = 'black'),
        axis.ticks.length = unit(8, 'points'),
        legend.position = "none")

ggsave("./figures/temporal/randomeffects.png", width = 10, height = 9, units = "in")

####################################################################################################
if (FALSE) {
  load("./models/temporal/inter_acc.RData")
  
  trial_types <- expand.grid(cue_type = c("valid", "noise", "invalid"),
                             is_mask_overlap = c(-1, 0, 1))
  trial_types$cue_m <- recode(trial_types$cue_type, "'valid' = -1/2; 'noise' = 0; 'invalid' = 1/2", as.factor.result = F)
  trial_types$mask_c <- recode(trial_types$is_mask_overlap, "-1 = -1/2; else = 1/2", as.factor.result = F)
  trial_types$mask_m <- recode(trial_types$is_mask_overlap, "-1 = -2/3; 0 = 1/3; 1 = 1/3", as.factor.result = F)
  trial_types$mask_r <- recode(trial_types$is_mask_overlap, "-1 = 0; 0 = -1/2; 1 = 1/2", as.factor.result = F)
}

y_preds <- predictSE.mer(inter_acc, newdata = trial_types, se.fit = T, type = "response")
preds <- cbind(trial_types, y_preds)
preds$upr <- with(preds, fit + se.fit)
preds$lwr <- with(preds, fit - se.fit)

ggplot(preds, aes(y = fit, ymin = lwr, ymax = upr, x = mask_c, color = cue_type)) +
  geom_pointrange(position = position_dodge(width = -0.1), size = 1.0) +
  geom_line(position = position_dodge(width = -0.1), size = 1.0) +
  coord_cartesian(xlim = c(-1, 1), ylim = c(0.9, 1.0)) +
  scale_y_continuous("Accuracy", label = percent) +
  scale_x_continuous("", breaks = c(-0.5, 0.5), labels = c("No Mask", "Mask")) +
  scale_color_discrete("Cue Type", labels = c("Valid", "Noise", "Invalid")) +
  guides(color = guide_legend(reverse = T)) +
  theme(text = element_text(color = 'black', size = 18),
        line = element_line(color = 'black'),
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = 'gray', linetype = 3, size = 0.6),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = 'black'),
        axis.text = element_text(color = 'black'),
        axis.ticks.x = element_blank(),
        axis.ticks.y = element_line(color = 'black'),
        axis.ticks.length = unit(8, 'points'),
        legend.position = c(0.16, 0.88),
        legend.background = element_blank(),
        legend.key = element_blank())

####################################################################################################
if (FALSE) {
  load("./models/temporal/mtype_rt.RData")
  
  trial_types <- expand.grid(cue_type = c("valid", "noise", "invalid"),
                             is_mask_overlap = c(-1, 0, 1))
  trial_types$cue_m <- recode(trial_types$cue_type, "'valid' = -1/2; 'noise' = 0; 'invalid' = 1/2", as.factor.result = F)
  trial_types$mask_c <- recode(trial_types$is_mask_overlap, "-1 = -1/2; else = 1/2", as.factor.result = F)
  trial_types$mask_m <- recode(trial_types$is_mask_overlap, "-1 = -2/3; 0 = 1/3; 1 = 1/3", as.factor.result = F)
  trial_types$mask_r <- recode(trial_types$is_mask_overlap, "-1 = 0; 0 = -1/2; 1 = 1/2", as.factor.result = F)
}

y_preds <- predictSE.mer(mtype_rt, newdata = trial_types, se.fit = T, type = "response")
preds <- cbind(trial_types, y_preds)
preds$upr <- with(preds, fit + se.fit)
preds$lwr <- with(preds, fit - se.fit)

preds$mask_pos <- recode(preds$is_mask_overlap, "-1 = -1/2; 1 = 1/4; 0 = 1/2", as.factor.result = F)

ggplot(preds, aes(y = fit, ymin = lwr, ymax = upr, x = mask_pos, color = cue_type)) +
  geom_pointrange(position = position_dodge(width = -0.1), size = 1.0) +
  geom_line(position = position_dodge(width = -0.1), size = 1.0) +
  coord_cartesian(xlim = c(-1, 1), ylim = c(400, 600)) +
  scale_y_continuous("Response Time (ms)") +
  scale_x_continuous("", breaks = c(-0.5, 0.25, 0.5), labels = c("No Mask", "Mask\n(During)", "Mask\n(After)")) +
  scale_color_discrete("Cue Type", labels = c("Valid", "Noise", "Invalid")) +
  guides(color = guide_legend(reverse = T)) +
  theme(text = element_text(color = 'black', size = 18),
        line = element_line(color = 'black'),
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = 'gray', linetype = 3, size = 0.6),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = 'black'),
        axis.text = element_text(color = 'black'),
        axis.ticks.x = element_blank(),
        axis.ticks.y = element_line(color = 'black'),
        axis.ticks.length = unit(8, 'points'),
        legend.position = c(0.16, 0.88),
        legend.background = element_blank(),
        legend.key = element_blank())

ggsave("figures/temporal/reactiontime2.png", width = 6, height = 6, units = "in")

####################################################################################################
if (FALSE) {
  load("./models/temporal/mtype_acc.RData")
  
  trial_types <- expand.grid(cue_type = c("valid", "noise", "invalid"),
                             is_mask_overlap = c(-1, 0, 1))
  trial_types$cue_m <- recode(trial_types$cue_type, "'valid' = -1/2; 'noise' = 0; 'invalid' = 1/2", as.factor.result = F)
  trial_types$mask_c <- recode(trial_types$is_mask_overlap, "-1 = -1/2; else = 1/2", as.factor.result = F)
  trial_types$mask_m <- recode(trial_types$is_mask_overlap, "-1 = -2/3; 0 = 1/3; 1 = 1/3", as.factor.result = F)
  trial_types$mask_r <- recode(trial_types$is_mask_overlap, "-1 = 0; 0 = -1/2; 1 = 1/2", as.factor.result = F)
}

y_preds <- predictSE.mer(mtype_acc, newdata = trial_types, se.fit = T, type = "response")
preds <- cbind(trial_types, y_preds)
preds$upr <- with(preds, fit + se.fit)
preds$lwr <- with(preds, fit - se.fit)

preds$mask_pos <- recode(preds$is_mask_overlap, "-1 = -1/2; 1 = 1/4; 0 = 1/2", as.factor.result = F)

ggplot(preds, aes(y = fit, ymin = lwr, ymax = upr, x = mask_pos, color = cue_type)) +
  geom_pointrange(position = position_dodge(width = -0.1), size = 1.0) +
  geom_line(position = position_dodge(width = -0.1), size = 1.0) +
  coord_cartesian(xlim = c(-1, 1), ylim = c(0.9, 1.0)) +
  scale_y_continuous("Accuracy", label = percent) +
  scale_x_continuous("", breaks = c(-0.5, 0.25, 0.5), labels = c("No Mask", "Mask\n(During)", "Mask\n(After)")) +
  scale_color_discrete("Cue Type", labels = c("Valid", "Noise", "Invalid")) +
  guides(color = guide_legend(reverse = T)) +
  theme(text = element_text(color = 'black', size = 18),
        line = element_line(color = 'black'),
        panel.background = element_blank(),
        panel.grid.major.y = element_line(color = 'gray', linetype = 3, size = 0.6),
        panel.grid.minor = element_blank(),
        axis.line = element_line(color = 'black'),
        axis.text = element_text(color = 'black'),
        axis.ticks.x = element_blank(),
        axis.ticks.y = element_line(color = 'black'),
        axis.ticks.length = unit(8, 'points'),
        legend.position = c(0.16, 0.88),
        legend.background = element_blank(),
        legend.key = element_blank())

ggsave("figures/temporal/accuracy2.png", width = 6, height = 6, units = "in")
