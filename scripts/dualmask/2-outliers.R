...
library(gridExtra) # arrange two ggplots
...
accs <- ggplot(subjs, aes(x = rank_acc, y = acc, label = subj_id, color = subj_id)) +
  geom_point() +
  geom_text(hjust = 0, vjust = -0.5, angle = 45, size = 4) +
  coord_cartesian(xlim = c(0.5, 28), ylim = c(0.7, 1.12)) +
  scale_x_continuous("Subject (Ranked by Accuracy)", breaks = c(1, seq(5, 25, by = 5))) +
  scale_y_continuous("Accuracy", label = percent, breaks = seq(0,1,by=0.1)) +
  theme(legend.position = "none")

(outliers <- arrangeGrob(rts, accs, nrow = 2))

ggsave("./dualmask/figure/2-outliers.png", plot = outliers, width = 12, height = 8, units = "in")
