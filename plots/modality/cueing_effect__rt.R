source("models/modality/cueing_effect__rt_for_plot.R")

library(AICcmodavg)
x_preds <- unique(modality[,c("cue_type", "cue_l", "cue_q", "mask_type", "helmert_main", "helmert_residual")])
...
means_modality <- dcast(preds, mask_type + mask_c ~ cue_type, value.var = "rt")
rect_points_modality <- means_modality %>%
  split(.$mask_type) %>%
  map(make_rects_modality) %>%
  rbind_all(.)

# ------------------------------------------------------------------------------
# Create the error lines.
# - requires grouping columns for mask_type and cue_type and (x, y) columns
modality_error <- dcast(preds, mask_type + mask_c ~ cue_type, value.var = "se")
modality_error_points <- means_modality %>% 
  select(-nocue) %>%
  mutate(
    minimum = valid - modality_error$valid,
    maximum = invalid + modality_error$invalid,
    valid = valid + modality_error$valid,
    invalid = invalid - modality_error$invalid
  )

modality_error_lines <- modality_error_points %>% 
  melt(id.vars = c("mask_type", "mask_c")) %>%
  arrange(mask_type, value)

# hack!
modality_error_lines$cue_type <- rep(c("valid", "valid", "invalid", "invalid"), times=3)
modality_error_lines$group <- with(modality_error_lines, paste(mask_type, cue_type, sep = ":"))
modality_error_lines$variable <- rep(c("min", "max"), times = 6)

modality_error_bars <- modality_error_lines %>% dcast(mask_c + group ~ variable, value.var = "value")
...
# ------------------------------------------------------------------------------
# Make the modality plot

modality_gg <- magnet_plot(preds, rect_points_modality, modality_error_bars, c(-0.6, 0.375)) +
  scale_x_continuous("", breaks = c(-0.6, 0.15, 0.6),
                     labels = c("Blank screen", "Auditory interference", "Visual interference"))
modality_gg


png("plots/modality/cueing_effect__rt.png", width = 6, height = 6, units = "in", res = 200)
print(modality_gg)
dev.off()